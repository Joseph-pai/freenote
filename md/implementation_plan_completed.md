  ...
  match /databases/{database}/documents {
    ...
    match /friendships/{id} {
      ...
      allow create: if isAuth() &&
                    isOwner(request.resource.data.initiatorId);
    ...
    match /invitationCodes/{code} {
      ...
      allow update: if isAuth();   // 允許任何人標記邀請碼為已使用
...
### Step 10：啟用 Cloud Functions 並設定邀請碼邏輯
...
4. 展開「**Environment variables**」→ 逐一填入所有 `NEXT_PUBLIC_FIREBASE_*` 變數（與 `.env.local` 相同）
5. 點擊「**Deploy site**」

> [!IMPORTANT]
> 部署完成後，回到 Firebase Console → Authentication → Authorized domains，加入您的 Netlify 網域（格式：`your-site-name.netlify.app`）。否則 Google 登入會失敗！

---

### Step 12：本地開發環境（使用 Firebase Emulators）

```bash
# 啟動 Firebase 模擬器（本地開發，不消耗配額）
...
| 邀請碼暴力猜測 | 邀請碼為 8 位，每用戶每小時最多生成 10 個 |
